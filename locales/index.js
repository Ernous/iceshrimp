...
const customDir = process.env.ICESHRIMP_CUSTOM_DIR ?? `${rootDir}/custom`;
...
readdirSync(`${customDir}/locales`).forEach((file) => {
	if (file.includes(".yml")) {
		file = file.slice(0, file.indexOf("."));
...
const locales = languages.reduce(
	(a, c) => (
		(a[c] =
			load(clean(readFileSync(`${import.meta.dirname}/${c}.yml`, "utf-8"))) ||
			{}),
		a
	),
	{},
...
const locales_custom = languages_custom.reduce(
	(a, c) => (
		(a[c] =
			load(
				clean(
					readFileSync(`${customDir}/locales/${c}.yml`, "utf-8"),
				),
			) || {}),
		a
	),
