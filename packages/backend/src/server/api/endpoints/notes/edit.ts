...
export const meta = {
	...
	errors: {
		...
		cannotAddPollToPost: {
			message: "You cannot add a poll to a post.",
			code: "CANNOT_ADD_POLL",
			id: "e6a00055-21c8-4d69-a4d3-c26731cf6f18",
		},

		cannotRemovePollFromPost: {
			message: "You cannot remove a poll from a post.",
			code: "CANNOT_REMOVE_POLL",
			id: "7c4889f1-cb50-4a36-a740-9d04f097479b",
		},
	}
} as const;
...
export default define(meta, paramDef, async (ps, user): Promise<Packed<"Note">> => {
	...
	if (!note.hasPoll && ps.poll) {
		throw new ApiError(meta.errors.cannotAddPollToPost);
	}

	if (note.hasPoll && !ps.poll) {
		throw new ApiError(meta.errors.cannotRemovePollFromPost);
	}
