<template>
	<div class="_formRoot">
		...
		<FormSelect
			...
			@update:modelValue="save()"
		>
			<template #label>{{ i18n.ts._biteControls.name }}</template>
			<option value="anyone">{{ i18n.ts._biteControls.anyone }}</option>
			<option value="followers">{{ i18n.ts._biteControls.followers }}</option>
			<option value="nobody">{{ i18n.ts._biteControls.nobody }}</option>
		</FormSelect>
	</div>
</template>
